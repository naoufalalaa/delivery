...
if(empty($_SESSION['id']) OR empty($_SESSION['cat'])){
    ...
    $getidprod=$_GET['id_prod'];
       $produit=$bdd->query('SELECT * FROM produits WHERE id_prod='.$getidprod);
       $prod=$produit->fetch();
    ...
    if(isset($_POST['add'])) {
           ...
           $ins = $bdd->prepare('UPDATE produits SET nom_produit=?, nom_f=?, cat=?, descript=?, ration=?,  prix=? WHERE ip_prod=?');
           $ins->execute(array($produit_titre,$produit_fournisseur,$cate, $produit_descripion,$ration ,$produit_prix,$getidprod));
     
           $message = '<div class="alert alert-success" role="alert\>
           <h4 class="alert-heading">Produit modifié!</h4>
...
<div align="center" class="uk-width-expand@m">
        ...
        ?>
        <h1 class="uk-text-lead">Modifier produit: <strong> <?=$prod['nom_produit']?></strong> <span class="uk-label uk-label-success"></span></h1>
        <div>
            ...
            <div class="form-row mb-4 d-flex justify-content-center">
                <div class="col-md-6">
                <input name="nomprod" type="text" value="<?=$prod['nom_produit'] ?>" class="form-control" id="validationCustom01" placeholder="Nom du Produit" required>
            ...
            <div class="form-row mb-4 d-flex justify-content-center">
            <div class="col-md-3">
                <input name="prix" type="text" value="<?=$prod['prix'] ?>" class="form-control" id="validationCustom01" placeholder="Prix du Produit" required>
                <div class="valid-feedback">
                    Looks good!
                </div>
            </div>
            <div class="col-md-3">
                ...
                <option selected value="<?=$prod['nom_f'] ?>"><?=$prod['nom_f'] ?></option>
                    <?php
            ...
            <div class="form-row mb-4 d-flex justify-content-center">
            <div class="col-md-3">
                <textarea name="produit_descript" class="form-control" id="validationCustom01"  placeholder="Description du Produit" required><?=$prod['descript'] ?></textarea>
                <div class="valid-feedback">
                    Looks good!
                </div>
            </div>
            <div class="col-md-3">
                <input name="ration" type="number" class="form-control" id="validationCustom01" value="<?=$prod['ration'] ?>" placeholder="Combien de personnes peuvent manger" required>
                <div class="valid-feedback">
                    Looks good!
                </div>
            </div>
            ...
            <ul class="uk-list uk-list-divider">
                <li>Image 1 <input name="produit" type="file"></li>
            ...
            <button name="add" class="btn btn-light my-4 btn-block col-md-3 d-flex justify-content-center" style="background-color:#e6e6e6" type="submit">Modifier l'article</button></div>
            </form>
