...
if(empty($_SESSION['id']) OR empty($_SESSION['cat'])){
    ...
    if(isset($_GET['C']) AND isset($_GET['O'])){
        $C=$_GET['C'];
        $O=$_GET['O'];
      $produit = $bdd->query('SELECT * FROM produits ORDER BY '.$C.' '.$O);
      }
      else{
      $produit = $bdd->query('SELECT * FROM produits ORDER BY date_ajout DESC');
      }
?>
<!DOCTYPE html>
<html lang="en">
...
<div class="uk-grid-divider uk-child-width-expand@s text-muted" style="margin-top:50px" uk-grid="parallax: 50">
                    ...
                    <ul class="uk-nav uk-nav-primary uk-margin-auto-vertical" align="left">
                        ...
                        <li class="uk-parent">
                            ...
                            <ul class="uk-nav-sub">
                                ...
                                <li><a href="index.php"><span uk-icon="pencil"></span> Gérer les Commandes</a></li>
                            </ul>
                        </li>
                    </ul>
    ...
    <div align="center" class="uk-width-expand@m">
        <h1 class="uk-text-lead">Produits <span class="uk-label uk-label-success"></span></h1>
        <div class="table-responsive">
                <table class="table table-hover">
                    <thead>
                        <tr>
                        <th scope="col">#</th>
                        <th scope="col"><a href="produits.php?C=nom_produit&O=<?php if(isset($O) AND isset($C) AND $C=="nom_produit" AND $O=="ASC"){echo'DESC';} else{?>ASC<?php }?>"> Nom produit <?php if(isset($O) AND isset($C) AND $C=="nom_produit" AND $O=="DESC" ){echo' <i class="fas fa-sort-up"></i>';} else{echo' <i class="fas fa-sort-down"></i>';} ?></a></th>
                        <th scope="col"><a href="produits.php?C=nom_f&O=<?php if(isset($O) AND isset($C) AND $C=="nom_f" AND $O=="ASC"){echo'DESC';} else{?>ASC<?php }?>">Fournisseur <?php if(isset($O) AND isset($C) AND $C=="nom_f" AND $O=="DESC" ){echo' <i class="fas fa-sort-up"></i>';} else{echo' <i class="fas fa-sort-down"></i>';} ?></a></th>
                        <th scope="col"><a href="produits.php?C=prix&O=<?php if(isset($O) AND isset($C) AND $C=="prix" AND $O=="ASC"){echo'DESC';} else{?>ASC<?php }?>">Prix <?php if(isset($O) AND isset($C) AND $C=="prix" AND $O=="DESC" ){echo' <i class="fas fa-sort-up"></i>';} else{echo' <i class="fas fa-sort-down"></i>';} ?></a></th>
                        <th scope="col"><a href="produits.php?C=ration&O=<?php if(isset($O) AND isset($C) AND $C=="ration" AND $O=="ASC"){echo'DESC';} else{?>ASC<?php }?>"> Ration <?php if(isset($O) AND isset($C) AND $C=="ration" AND $O=="DESC" ){echo' <i class="fas fa-sort-up"></i>';} else{echo' <i class="fas fa-sort-down"></i>';} ?></a></th>
                        <th scope="col"><a href="produits.php?C=cat&O=<?php if(isset($O) AND isset($C) AND $C=="cat" AND $O=="ASC"){echo'DESC';} else{?>ASC<?php }?>"> Catégorie <?php if(isset($O) AND isset($C) AND $C=="cat" AND $O=="DESC" ){echo' <i class="fas fa-sort-up"></i>';} else{echo' <i class="fas fa-sort-down"></i>';} ?></a></th>
                        <th scope="col"><a href="produits.php?C=nbr_commande&O=<?php if(isset($O) AND isset($C) AND $C=="nbr_commande" AND $O=="ASC"){echo'DESC';} else{?>ASC<?php }?>">Commandes <?php if(isset($O) AND isset($C) AND $C=="nbr_commande" AND $O=="DESC" ){echo' <i class="fas fa-sort-up"></i>';} else{echo' <i class="fas fa-sort-down"></i>';} ?></a></th>
                        <th scope="col"><a href="produits.php?C=date_ajout&O=<?php if(isset($O) AND isset($C) AND $C=="date_ajout" AND $O=="ASC"){echo'DESC';} else{?>ASC<?php }?>"> Date d'ajout <?php if(isset($O) AND isset($C) AND $C=="date_ajout" AND $O=="DESC" ){echo' <i class="fas fa-sort-up"></i>';} else{echo' <i class="fas fa-sort-down"></i>';} ?></a></th>
                    ...
                    <?php 
                        ...
                        while($prod=$produit->fetch()){
                            $time=strtotime($prod['date_ajout']);
                            $productname=$prod['nom_produit'];
                            $nomf=$prod['nom_f'];
                            $descript=$prod['descript'];
                            $prix=$prod['prix'];
                            $ration=$prod['ration'];
                            $nbrc=$prod['nbr_commande'];
                            $cat=$prod['cat'];
                    ?>
                        <tr>
                        ...
                        <td><?=$nomf ?></td>
                        <td><?=$prix ?> Dhs</td>
                        <td><?=$ration ?> <i class="fas fa-users"></i></td>
                        <td><?=$cat ?></td>
                        <td><?=$nbrc ?></td>
                    <td>
                    ...
                    <td><a href="delete_prod.php?id_prod=<?=$prod['id_prod'] ?>" class="btn btn-danger"><i class="fas fa-trash-alt"></i> </a></td>
                    <td><a href="modifyprod.php?id_prod=<?=$prod['id_prod'] ?>" class="btn btn-primary"><i class="fas fa-edit"></i> </a></td>
